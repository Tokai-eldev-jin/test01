...
void setup() {
  ...
  size(960,515,JAVA2D);//ソフトの表示サイズ　　　リテラルのみ
  //fullScreen();
  ...
  //ウィンドウの位置
  smoothCanvas.getFrame().setLocation(10, 10); //モニターのどこか
...
void draw() {
  ...
  //tint(200, 255, 255, 100);  // 塗り色は青、透明度を50%に設定
  //for(i=0;i<1;i++){
  //  for(ii=10;ii<11;ii++){
  //    image(img, i*195, 35*ii);
  //  }
  //}
  //tint(200, 255, 255, 256);  // 塗り色は青、透明度を50%に設定
  image(img,0, 605);
  ...
  if(mode==0){    //########## 1m表示の時 ##########
    ...
    for(i=0;i<=10;i++){
      text(100*page+10*i,screen_w/10*i+sc_offx,screen_h+sc_offy+20);
    }
    ...
    textAlign(CENTER);
    textSize(20);
    text("Distance(cm)", screen_w/10*5+sc_offx,screen_h+sc_offy+50);
    //##########　X軸のタイトル表示　##########
    ...
    for(i=1;i<=10;i++){
      text(100*0+10*i,screen_w/10*i+sc_offx,screen_h+sc_offy+15);
    }
    ...
    for(i=1;i<=10;i++){
      text(100*1+10*i,screen_w/10*i+sc_offx,screen_h+sc_offy+screen_h+15);
    }
    ...
    for(i=0;i<=10;i++){
      text(100*2+10*i,screen_w/10*i+sc_offx,screen_h+sc_offy+screen_h*2+15);
    }
    ...
    textAlign(CENTER);
    textSize(20);
    text("Distance(cm)", screen_w/10*5+sc_offx,screen_h*3+sc_offy+40);
    //##########　X軸のタイトル表示　##########
